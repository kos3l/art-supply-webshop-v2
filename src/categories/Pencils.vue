  ...
  <div class="product-page">
    <v-container fluid light class="pa-0">
      ...
      <v-row >
        ...
        <v-col sm="12" md="8" lg="9" xl="10" class="pr-10 itemColumn d-flex flex-wrap justify-start align-start" v-if="ctgr === 'pencil'">
            <ProductCardDrawing  v-for="drawingItem in pencilOnly" :drawingItem="drawingItem" :key="drawingItem.name"/>
        </v-col>
        <v-col sm="12" md="8" lg="9" xl="10" class="pr-10 itemColumn d-flex flex-wrap justify-start align-start" v-if="ctgr === 'crayon'">
            <ProductCardDrawing  v-for="drawingItem in crayonOnly" :drawingItem="drawingItem" :key="drawingItem.name"/>
        </v-col>
      </v-row>
    </v-container>
...
export default {
  ...
  computed: {
    ...
    pencilOnly() {
        ...
        return this.$store.state.drawingItems.filter(drawingfilter => drawingfilter.category === "pencil" )
    },
    crayonOnly() {
          return this.$store.state.drawingItems.filter(drawingfilter => drawingfilter.category === "crayon" )
    }
